...
class ApproachNet(nn.Module):
    ...
    def forward(self, seed_features, end_points):
        ...
        if self.is_training:
            ...
            top_view_inds = []
            for i in range(B):
                top_view_inds_batch = torch.multinomial(view_score_[i], 1, replacement=False)  # multinomial 大概率不支持
...
class CloudCrop(nn.Module):
    def __init__(self, nsample, seed_feature_dim, cylinder_radius=0.05, hmin=-0.02, hmax=0.04):
        ...
        self.mlps = pt_utils.SharedMLP(mlps, bn=True)  #  use conv2d to express linear, more hardware-friendly
