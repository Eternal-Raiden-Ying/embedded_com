...
from voice_service.runtime.asr_engine import (
    AudioCommandPipeline,
    OfflineASREngine,
    OnlineASREngine,
    create_asr_backend,
    merge_online_hypothesis,
)
...
def test_online_hypothesis_merge_handles_cumulative_incremental_overlap_and_empty(previous, incoming, expected):
    assert merge_online_hypothesis(previous, incoming) == expected
...
def test_online_final_merges_tail_before_nlu_result_is_finalized():
    engine = OnlineASREngine("", True, [5, 10, 5], 4, 1, dry_run_text=True)
    engine.dry_run_text = False
    engine.backend = _IncrementalBackend(["拿水", "瓶"])
    session = engine.create_session()

    chunk = engine.feed(session, [], is_final=False)
    final = engine.feed(session, [], is_final=True)

    assert chunk["raw_text"] == "拿水"
    assert chunk["merged_text"] == "拿水"
    assert final["raw_text"] == "瓶"
    assert final["previous_text"] == "拿水"
    assert final["merged_text"] == "拿水瓶"

    pipeline = object.__new__(AudioCommandPipeline)
    pipeline._interpret_text = lambda text, _conf, _latency, _samples: {"text": text}
    assert pipeline.finalize_stream_result(session)["text"] == "拿水瓶"
...
def _online_worker():
    ...
    worker.handled_results = []
    worker._handle_result = lambda result: (worker.handled_results.append(result) or {"keep_alive": False, "tts": ""})
    return worker
...
def test_start_chunk_final_only_final_reaches_decision_and_is_unique():
    ...
    assert len(worker.handled_results) == 1
