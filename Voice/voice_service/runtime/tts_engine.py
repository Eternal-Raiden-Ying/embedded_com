...
class PiperTTS:
    def __init__(self, model_onnx: str, cache_dir: str, out_dir: str,
                 mode: str = "save", play_cmd: str = "aplay -q", dry_run_text: bool = False):
        ...
        if self.dry_run_text:
            jlog({"level": "info", "src": "tts", "msg": "PiperTTS initialized in dry-run-text mode."})
            return
    ...
    def say(self, text: str) -> Optional[Path]:
        if self.dry_run_text:
            print(f"[TTS MOCK PLAYBACK] {text}", flush=True)
            jlog({"level": "info", "src": "tts", "event": "mock_playback", "text": text})
            return None
