...
def validate_config(config: SystemGlobalConfig, force_production: bool = False) -> None:
    """Validate config defaults to warn/fail-fast on dangerous defaults."""
    import sys
    warnings = []
    
    car = config.orchestrator.car
    dry_run = config.orchestrator.serial.dry_run
    ...
    is_main_app = (len(sys.argv) > 0 and 
                   (sys.argv[0].endswith("app/main.py") or 
                    sys.argv[0].endswith("app\\main.py") or 
                    "app.main" in sys.argv or 
                    "orchestrator_service.app.main" in sys.argv))
    
    if car.edge_slide_vy_mps == 0.14:
        msg = (
            "WARNING: edge_slide_vy_mps is at dangerous default value 0.14 m/s! "
            "Recommended is 0.010 m/s for safe base movement and to prevent overshoot."
        )
        if not dry_run and (force_production or (is_main_app and not is_test)):
            raise ValueError(f"CRITICAL CONFIG ERROR: {msg} (Fail-fast triggered in production mode)")
        else:
            warnings.append(msg)
