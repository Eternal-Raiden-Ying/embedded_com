...
def _sync_table_motion_absolute() -> None:
    # Absolute-speed configuration: all controller commands are already m/s or rad/s.
    CONFIG.car.table_vx_mps_min = abs(float(CONFIG.car.table_controlled_vx_min_mps))
    CONFIG.car.table_vx_mps_max = abs(float(CONFIG.car.table_controlled_vx_max_mps))
    CONFIG.car.table_vy_max_mps = abs(float(CONFIG.car.table_controlled_vy_max_mps))
    CONFIG.car.table_wz_view_max_radps = abs(float(CONFIG.car.table_controlled_wz_max_radps))
    CONFIG.car.table_wz_plane_max_radps = abs(float(CONFIG.car.table_coarse_align_wz_max_radps))
...
def _apply_stage_params(data: Dict[str, Any]) -> None:
    ...
    CONFIG.control.final_lock_enter_dist_th_m = float(
        table_docking.get("final_lock_enter_dist_th_m", CONFIG.control.final_lock_enter_dist_th_m)
    )
    ...
    CONFIG.car.yolo_table_max_wz_radps = float(
        yolo_table.get("yolo_table_max_wz_radps", yolo_table.get("yolo_max_wz_radps", yolo_table.get("max_wz", CONFIG.car.yolo_table_max_wz_radps)))
    )
    CONFIG.car.yolo_table_forward_vx_mps = float(
        yolo_table.get("yolo_table_forward_vx_mps", yolo_table.get("yolo_forward_vx_mps", CONFIG.car.yolo_table_forward_vx_mps))
    )
    CONFIG.car.search_table_wz_radps = float(
        yolo_table.get("rotate_search_wz_radps", yolo_table.get("search_table_wz_radps", CONFIG.car.search_table_wz_radps))
    )
    ...
    CONFIG.docking.approach_max_vx_mps = float(controlled.get("max_vx_mps", CONFIG.docking.approach_max_vx_mps))
    CONFIG.docking.approach_max_wz_radps = float(controlled.get("max_wz_radps", CONFIG.docking.approach_max_wz_radps))
    ...
    CONFIG.car.table_fov_soft_th = float(controlled.get("fov_soft_th", CONFIG.car.table_fov_soft_th))
    CONFIG.car.table_fov_hard_th = float(controlled.get("fov_hard_th", CONFIG.car.table_fov_hard_th))
    CONFIG.car.table_stage_a_wz_radps = float(controlled.get("stage_a_wz_radps", CONFIG.car.table_stage_a_wz_radps))
    CONFIG.car.table_stage_b_vx_max_mps = float(controlled.get("stage_b_vx_max_mps", CONFIG.car.table_stage_b_vx_max_mps))
    CONFIG.car.table_stage_c_vx_max_mps = float(controlled.get("stage_c_vx_max_mps", CONFIG.car.table_stage_c_vx_max_mps))
    CONFIG.car.table_min_forward_dist_err_m = float(controlled.get("min_forward_dist_err_m", CONFIG.car.table_min_forward_dist_err_m))
    CONFIG.car.table_vx_mps_min = float(controlled.get("vx_mps_min", controlled.get("min_vx_mps", CONFIG.car.table_vx_mps_min)))
    CONFIG.car.table_vx_mps_max = float(controlled.get("vx_mps_max", controlled.get("max_vx_mps", CONFIG.car.table_vx_mps_max)))
    CONFIG.car.table_vx_kp_mps_per_m = float(controlled.get("vx_kp_mps_per_m", controlled.get("dist_kp", CONFIG.car.table_vx_kp_mps_per_m)))
    CONFIG.car.table_yaw_slow_th_rad = float(controlled.get("yaw_slow_th_rad", controlled.get("yaw_slow_th", CONFIG.car.table_yaw_slow_th_rad)))
    CONFIG.car.table_yaw_stop_th_rad = float(controlled.get("yaw_stop_th_rad", controlled.get("yaw_stop_th", CONFIG.car.table_yaw_stop_th_rad)))
    CONFIG.car.table_near_dist_err_th_m = float(controlled.get("near_dist_err_th_m", CONFIG.car.table_near_dist_err_th_m))
    CONFIG.car.table_vy_max_mps = float(controlled.get("view_vy_max_mps", CONFIG.car.table_vy_max_mps))
    CONFIG.car.table_wz_view_max_radps = float(controlled.get("view_wz_max_radps", CONFIG.car.table_wz_view_max_radps))
    CONFIG.car.table_wz_plane_max_radps = float(controlled.get("plane_wz_max_radps", CONFIG.car.table_wz_plane_max_radps))
    CONFIG.car.table_view_recover_vy_mps = float(controlled.get("view_recover_vy_mps", CONFIG.car.table_view_recover_vy_mps))
    CONFIG.car.table_view_recover_wz_radps = float(controlled.get("view_recover_wz_radps", CONFIG.car.table_view_recover_wz_radps))
    CONFIG.car.table_view_wz_sign = float(controlled.get("view_wz_sign", CONFIG.car.table_view_wz_sign))
    CONFIG.car.table_view_vy_sign = float(controlled.get("view_vy_sign", CONFIG.car.table_view_vy_sign))
    CONFIG.car.table_plane_yaw_sign = float(controlled.get("plane_yaw_sign", CONFIG.car.table_plane_yaw_sign))

    def _motion_section(name: str) -> Dict[str, Any]:
        section = table_motion.get(name)
        return dict(section or {}) if isinstance(section, dict) else {}

    def _set_motion_value(attr: str, section: Dict[str, Any], key: str) -> None:
        if key in section:
            setattr(CONFIG.car, attr, float(section.get(key, getattr(CONFIG.car, attr))))

    coarse_motion = _motion_section("coarse_align")
    controlled_motion = _motion_section("controlled_approach")
    final_motion = _motion_section("final_lock")
    deadband_motion = _motion_section("deadband")
    if "pose_missing_safe_vx_mps" in table_motion:
    ...
    _set_motion_value("table_coarse_align_vx_max_mps", coarse_motion, "vx_max_mps")
    _set_motion_value("table_coarse_align_vy_min_mps", coarse_motion, "vy_min_mps")
    _set_motion_value("table_coarse_align_vy_max_mps", coarse_motion, "vy_max_mps")
    _set_motion_value("table_coarse_align_wz_min_radps", coarse_motion, "wz_min_radps")
    _set_motion_value("table_coarse_align_wz_max_radps", coarse_motion, "wz_max_radps")
    _set_motion_value("table_controlled_vx_min_mps", controlled_motion, "vx_min_mps")
    _set_motion_value("table_controlled_vx_max_mps", controlled_motion, "vx_max_mps")
    _set_motion_value("table_controlled_vy_min_mps", controlled_motion, "vy_min_mps")
    _set_motion_value("table_controlled_vy_max_mps", controlled_motion, "vy_max_mps")
    _set_motion_value("table_controlled_wz_min_radps", controlled_motion, "wz_min_radps")
    _set_motion_value("table_controlled_wz_max_radps", controlled_motion, "wz_max_radps")
    _set_motion_value("table_final_lock_vx_min_mps", final_motion, "vx_min_mps")
    _set_motion_value("table_final_lock_vx_max_mps", final_motion, "vx_max_mps")
    _set_motion_value("table_final_lock_vy_min_mps", final_motion, "vy_min_mps")
    _set_motion_value("table_final_lock_vy_max_mps", final_motion, "vy_max_mps")
    _set_motion_value("table_final_lock_wz_min_radps", final_motion, "wz_min_radps")
    _set_motion_value("table_final_lock_wz_max_radps", final_motion, "wz_max_radps")
    _set_motion_value("table_vx_deadband_mps", deadband_motion, "vx_mps")
    _set_motion_value("table_vy_deadband_mps", deadband_motion, "vy_mps")
    _set_motion_value("table_wz_deadband_radps", deadband_motion, "wz_radps")
    _sync_table_motion_absolute()

    CONFIG.car.edge_slide_vy_mps = float(edge_slide.get("slide_vy_mps", CONFIG.car.edge_slide_vy_mps))
    CONFIG.car.edge_slide_weak_vy_mps = float(edge_slide.get("weak_slide_vy_mps", CONFIG.car.edge_slide_weak_vy_mps))
    CONFIG.car.edge_slide_max_vx_mps = float(edge_slide.get("max_vx_correction_mps", CONFIG.car.edge_slide_max_vx_mps))
    CONFIG.car.edge_slide_max_wz_radps = float(edge_slide.get("max_wz_correction_radps", CONFIG.car.edge_slide_max_wz_radps))
    CONFIG.car.edge_slide_dist_kp_mps_per_m = float(edge_slide.get("dist_kp", CONFIG.car.edge_slide_dist_kp_mps_per_m))
    CONFIG.car.edge_slide_yaw_kp_radps_per_rad = float(edge_slide.get("yaw_kp", CONFIG.car.edge_slide_yaw_kp_radps_per_rad))
    ...
    CONFIG.control.edge_slide_dist_out_of_range_hold_s = _ms_to_s(edge_slide.get("dist_out_of_range_hold_ms"), CONFIG.control.edge_slide_dist_out_of_range_hold_s)
    CONFIG.control.edge_slide_max_relock_attempts = int(edge_slide.get("max_relock_attempts", CONFIG.control.edge_slide_max_relock_attempts))
    CONFIG.control.edge_slide_relock_failure_is_fatal = bool(edge_slide.get("relock_failure_is_fatal", CONFIG.control.edge_slide_relock_failure_is_fatal))
    ...
    CONFIG.control.table_obs_stale_soft_ms = int(edge_follow.get("table_obs_stale_soft_ms", CONFIG.control.table_obs_stale_soft_ms))
    CONFIG.control.table_obs_stale_stop_ms = int(edge_follow.get("table_obs_stale_stop_ms", CONFIG.control.table_obs_stale_stop_ms))
    CONFIG.control.table_obs_stale_hard_ms = int(edge_follow.get("table_obs_stale_hard_ms", CONFIG.control.table_obs_stale_hard_ms))
    CONFIG.control.table_step_mode_enable = bool(edge_follow.get("table_step_mode_enable", CONFIG.control.table_step_mode_enable))
    CONFIG.control.table_step_burst_ms = int(edge_follow.get("table_step_burst_ms", CONFIG.control.table_step_burst_ms))
    CONFIG.control.table_step_hold_until_new_obs = bool(edge_follow.get("table_step_hold_until_new_obs", CONFIG.control.table_step_hold_until_new_obs))
...
def _apply_car_cmd_params(data: Dict[str, Any]) -> None:
    car_cmd = dict(data.get("car_cmd") or data.get("car_control") or data or {})
    CONFIG.car.send_period_ms = int(car_cmd.get("send_period_ms", CONFIG.car.send_period_ms))
    if "uart_keepalive_hz" in car_cmd:
        CONFIG.car.uart_keepalive_hz = float(car_cmd.get("uart_keepalive_hz", CONFIG.car.uart_keepalive_hz))
    elif CONFIG.car.send_period_ms > 0:
        CONFIG.car.uart_keepalive_hz = 1000.0 / float(CONFIG.car.send_period_ms)
    CONFIG.car.min_uart_keepalive_hz = float(car_cmd.get("min_uart_keepalive_hz", CONFIG.car.min_uart_keepalive_hz))
    CONFIG.car.uart_keepalive_hz = max(float(CONFIG.car.min_uart_keepalive_hz), float(CONFIG.car.uart_keepalive_hz))
    ...
    CONFIG.car.max_vx_mps = float(car_cmd.get("max_vx_mps", CONFIG.car.max_vx_mps))
    CONFIG.car.max_vy_mps = float(car_cmd.get("max_vy_mps", CONFIG.car.max_vy_mps))
    CONFIG.car.max_wz_radps = float(car_cmd.get("max_wz_radps", CONFIG.car.max_wz_radps))
...
CONFIG.serial.dry_run = _env_bool("ORCH_SERIAL_DRY_RUN", CONFIG.serial.dry_run)
...
CONFIG.control.table_approach_warmup_s = _env_float("ORCH_TABLE_APPROACH_WARMUP_S", CONFIG.control.table_approach_warmup_s)
...
CONFIG.control.final_lock_enter_dist_th_m = _env_float("ORCH_TABLE_FINAL_LOCK_ENTER_DIST_TH_M", CONFIG.control.final_lock_enter_dist_th_m)
CONFIG.control.final_lock_enter_yaw_th_rad = _env_float("ORCH_TABLE_FINAL_LOCK_ENTER_YAW_TH_RAD", CONFIG.control.final_lock_enter_yaw_th_rad)
...
CONFIG.control.table_obs_stale_soft_ms = _env_int("ORCH_TABLE_OBS_STALE_SOFT_MS", CONFIG.control.table_obs_stale_soft_ms)
CONFIG.control.table_obs_stale_stop_ms = _env_int("ORCH_TABLE_OBS_STALE_STOP_MS", CONFIG.control.table_obs_stale_stop_ms)
CONFIG.control.table_obs_stale_hard_ms = _env_int("ORCH_TABLE_OBS_STALE_HARD_MS", CONFIG.control.table_obs_stale_hard_ms)
CONFIG.control.table_step_mode_enable = _env_bool("ORCH_TABLE_STEP_MODE_ENABLE", CONFIG.control.table_step_mode_enable)
CONFIG.control.table_step_burst_ms = _env_int("ORCH_TABLE_STEP_BURST_MS", CONFIG.control.table_step_burst_ms)
CONFIG.control.table_step_hold_until_new_obs = _env_bool("ORCH_TABLE_STEP_HOLD_UNTIL_NEW_OBS", CONFIG.control.table_step_hold_until_new_obs)
...
CONFIG.control.edge_slide_dist_out_of_range_hold_s = _env_float("ORCH_EDGE_SLIDE_DIST_OUT_OF_RANGE_HOLD_S", CONFIG.control.edge_slide_dist_out_of_range_hold_s)
CONFIG.control.edge_slide_max_relock_attempts = _env_int("ORCH_EDGE_SLIDE_MAX_RELOCK_ATTEMPTS", CONFIG.control.edge_slide_max_relock_attempts)
CONFIG.control.edge_slide_relock_failure_is_fatal = _env_bool("ORCH_EDGE_SLIDE_RELOCK_FAILURE_IS_FATAL", CONFIG.control.edge_slide_relock_failure_is_fatal)
...
CONFIG.control.relocate_turn_s = _env_float("ORCH_RELOCATE_TURN_WZ_RADPS_S", CONFIG.control.relocate_turn_s)
...
CONFIG.car.search_table_wz_radps = _env_float("ORCH_SEARCH_TABLE_WZ_RADPS", CONFIG.car.search_table_wz_radps)
CONFIG.car.fallback_align_turn_wz_min_radps = _env_float("ORCH_FALLBACK_TURN_WZ_MIN_RADPS", CONFIG.car.fallback_align_turn_wz_min_radps)
CONFIG.car.fallback_align_turn_wz_max_radps = _env_float("ORCH_FALLBACK_TURN_WZ_MAX_RADPS", CONFIG.car.fallback_align_turn_wz_max_radps)
CONFIG.car.fallback_forward_vx_mps_min = _env_float("ORCH_FALLBACK_VX_MIN_MPS", CONFIG.car.fallback_forward_vx_mps_min)
CONFIG.car.fallback_forward_vx_mps_max = _env_float("ORCH_FALLBACK_VX_MAX_MPS", CONFIG.car.fallback_forward_vx_mps_max)
...
CONFIG.car.return_turn_wz_min_radps = _env_float("ORCH_RETURN_TURN_WZ_MIN_RADPS", CONFIG.car.return_turn_wz_min_radps)
CONFIG.car.return_turn_wz_max_radps = _env_float("ORCH_RETURN_TURN_WZ_MAX_RADPS", CONFIG.car.return_turn_wz_max_radps)
CONFIG.car.return_vx_mps_min = _env_float("ORCH_RETURN_VX_MIN_MPS", CONFIG.car.return_vx_mps_min)
CONFIG.car.return_vx_mps_max = _env_float("ORCH_RETURN_VX_MAX_MPS", CONFIG.car.return_vx_mps_max)
CONFIG.car.edge_slide_vy_mps = _env_float("ORCH_EDGE_SLIDE_VY_MPS", CONFIG.car.edge_slide_vy_mps)
CONFIG.car.edge_slide_weak_vy_mps = _env_float("ORCH_EDGE_SLIDE_WEAK_VY_MPS", CONFIG.car.edge_slide_weak_vy_mps)
CONFIG.car.edge_slide_dist_kp_mps_per_m = _env_float("ORCH_EDGE_SLIDE_DIST_KP", CONFIG.car.edge_slide_dist_kp_mps_per_m)
CONFIG.car.edge_slide_yaw_kp_radps_per_rad = _env_float("ORCH_EDGE_SLIDE_YAW_KP", CONFIG.car.edge_slide_yaw_kp_radps_per_rad)
CONFIG.car.edge_slide_max_vx_mps = _env_float("ORCH_EDGE_SLIDE_MAX_VX_MPS", CONFIG.car.edge_slide_max_vx_mps)
CONFIG.car.edge_slide_max_wz_radps = _env_float("ORCH_EDGE_SLIDE_MAX_WZ_RADPS", CONFIG.car.edge_slide_max_wz_radps)
CONFIG.car.leave_edge_vx_mps = _env_float("ORCH_LEAVE_EDGE_VX_MPS", CONFIG.car.leave_edge_vx_mps)
CONFIG.car.relocate_turn_wz_radps = _env_float("ORCH_RELOCATE_TURN_WZ_RADPS", CONFIG.car.relocate_turn_wz_radps)
CONFIG.car.avoid_turn_wz_radps = _env_float("ORCH_AVOID_TURN_WZ_RADPS", CONFIG.car.avoid_turn_wz_radps)
CONFIG.car.avoid_reverse_vx_mps = _env_float("ORCH_AVOID_REVERSE_VX_MPS", CONFIG.car.avoid_reverse_vx_mps)
...
if CONFIG.car.send_period_ms > 0 and "ORCH_UART_KEEPALIVE_HZ" not in os.environ:
    CONFIG.car.uart_keepalive_hz = 1000.0 / float(CONFIG.car.send_period_ms)
CONFIG.car.uart_keepalive_hz = _env_float("ORCH_UART_KEEPALIVE_HZ", CONFIG.car.uart_keepalive_hz)
CONFIG.car.min_uart_keepalive_hz = _env_float("ORCH_MIN_UART_KEEPALIVE_HZ", CONFIG.car.min_uart_keepalive_hz)
CONFIG.car.uart_keepalive_hz = max(float(CONFIG.car.min_uart_keepalive_hz), float(CONFIG.car.uart_keepalive_hz))
CONFIG.runtime.tick_hz = max(float(CONFIG.runtime.tick_hz), float(CONFIG.car.uart_keepalive_hz))
CONFIG.car.max_vx_mps = _env_float("ORCH_CAR_MAX_VX", CONFIG.car.max_vx_mps)
CONFIG.car.max_vy_mps = _env_float("ORCH_CAR_MAX_VY", CONFIG.car.max_vy_mps)
CONFIG.car.max_wz_radps = _env_float("ORCH_CAR_MAX_WZ", CONFIG.car.max_wz_radps)
CONFIG.car.table_min_forward_dist_err_m = _env_float("ORCH_TABLE_MIN_FORWARD_DIST_ERR_M", CONFIG.car.table_min_forward_dist_err_m)
CONFIG.car.table_vx_mps_min = _env_float("ORCH_TABLE_VX_MIN_MPS", CONFIG.car.table_vx_mps_min)
CONFIG.car.table_vx_mps_max = _env_float("ORCH_TABLE_VX_MAX_MPS", CONFIG.car.table_vx_mps_max)
CONFIG.car.table_vx_kp_mps_per_m = _env_float("ORCH_TABLE_VX_KP_MPS_PER_M", CONFIG.car.table_vx_kp_mps_per_m)
CONFIG.car.table_coarse_align_vx_max_mps = _env_float("ORCH_TABLE_COARSE_VX_MAX_MPS", CONFIG.car.table_coarse_align_vx_max_mps)
CONFIG.car.table_coarse_align_vy_min_mps = _env_float("ORCH_TABLE_COARSE_VY_MIN_MPS", CONFIG.car.table_coarse_align_vy_min_mps)
CONFIG.car.table_coarse_align_vy_max_mps = _env_float("ORCH_TABLE_COARSE_VY_MAX_MPS", CONFIG.car.table_coarse_align_vy_max_mps)
CONFIG.car.table_coarse_align_wz_min_radps = _env_float("ORCH_TABLE_COARSE_WZ_MIN_RADPS", CONFIG.car.table_coarse_align_wz_min_radps)
CONFIG.car.table_coarse_align_wz_max_radps = _env_float("ORCH_TABLE_COARSE_WZ_MAX_RADPS", CONFIG.car.table_coarse_align_wz_max_radps)
CONFIG.car.table_controlled_vx_min_mps = _env_float("ORCH_TABLE_CONTROLLED_VX_MIN_MPS", CONFIG.car.table_controlled_vx_min_mps)
CONFIG.car.table_controlled_vx_max_mps = _env_float("ORCH_TABLE_CONTROLLED_VX_MAX_MPS", CONFIG.car.table_controlled_vx_max_mps)
CONFIG.car.table_controlled_vy_min_mps = _env_float("ORCH_TABLE_CONTROLLED_VY_MIN_MPS", CONFIG.car.table_controlled_vy_min_mps)
...
CONFIG.car.table_final_lock_vx_max_mps = _env_float("ORCH_TABLE_FINAL_VX_MAX_MPS", CONFIG.car.table_final_lock_vx_max_mps)
CONFIG.car.table_final_lock_vy_min_mps = _env_float("ORCH_TABLE_FINAL_VY_MIN_MPS", CONFIG.car.table_final_lock_vy_min_mps)
CONFIG.car.table_final_lock_vy_max_mps = _env_float("ORCH_TABLE_FINAL_VY_MAX_MPS", CONFIG.car.table_final_lock_vy_max_mps)
CONFIG.car.table_final_lock_wz_min_radps = _env_float("ORCH_TABLE_FINAL_WZ_MIN_RADPS", CONFIG.car.table_final_lock_wz_min_radps)
CONFIG.car.table_final_lock_wz_max_radps = _env_float("ORCH_TABLE_FINAL_WZ_MAX_RADPS", CONFIG.car.table_final_lock_wz_max_radps)
CONFIG.car.table_vx_deadband_mps = _env_float("ORCH_TABLE_VX_DEADBAND_MPS", CONFIG.car.table_vx_deadband_mps)
CONFIG.car.table_vy_deadband_mps = _env_float("ORCH_TABLE_VY_DEADBAND_MPS", CONFIG.car.table_vy_deadband_mps)
CONFIG.car.table_wz_deadband_radps = _env_float("ORCH_TABLE_WZ_DEADBAND_RADPS", CONFIG.car.table_wz_deadband_radps)
CONFIG.car.table_yaw_slow_th_rad = _env_float("ORCH_TABLE_YAW_SLOW_TH_RAD", CONFIG.car.table_yaw_slow_th_rad)
CONFIG.car.table_yaw_stop_th_rad = _env_float("ORCH_TABLE_YAW_STOP_TH_RAD", CONFIG.car.table_yaw_stop_th_rad)
CONFIG.car.table_near_dist_err_th_m = _env_float("ORCH_TABLE_NEAR_DIST_ERR_TH_M", CONFIG.car.table_near_dist_err_th_m)
CONFIG.car.table_view_recover_vy_mps = _env_float("ORCH_TABLE_VIEW_RECOVER_VY_MPS", CONFIG.car.table_view_recover_vy_mps)
CONFIG.car.table_view_recover_wz_radps = _env_float("ORCH_TABLE_VIEW_RECOVER_WZ_RADPS", CONFIG.car.table_view_recover_wz_radps)
CONFIG.car.stm32_wheel_speed_limit = _env_int("ORCH_STM32_WHEEL_SPEED_LIMIT", CONFIG.car.stm32_wheel_speed_limit)
CONFIG.car.stm32_vx_scale = _env_float("ORCH_STM32_VX_SCALE", CONFIG.car.stm32_vx_scale)
CONFIG.car.stm32_vy_scale = _env_float("ORCH_STM32_VY_SCALE", CONFIG.car.stm32_vy_scale)
CONFIG.car.stm32_wz_scale = _env_float("ORCH_STM32_WZ_SCALE", CONFIG.car.stm32_wz_scale)
_sync_table_motion_absolute()
CONFIG.car.jog_forward_speed = _env_float("ORCH_JOG_FORWARD_SPEED", CONFIG.car.jog_forward_speed)
CONFIG.car.jog_turn_speed = _env_float("ORCH_JOG_TURN_SPEED", CONFIG.car.jog_turn_speed)
CONFIG.car.jog_duration_ms = _env_int("ORCH_JOG_DURATION_MS", CONFIG.car.jog_duration_ms)
...
CONFIG.docking.coarse_max_wz_radps = _env_float("ORCH_DOCKING_COARSE_MAX_WZ", CONFIG.docking.coarse_max_wz_radps)
CONFIG.docking.approach_max_vx_mps = _env_float("ORCH_DOCKING_APPROACH_MAX_VX", CONFIG.docking.approach_max_vx_mps)
CONFIG.docking.approach_max_vy_mps = _env_float("ORCH_DOCKING_APPROACH_MAX_VY", CONFIG.docking.approach_max_vy_mps)
CONFIG.docking.approach_max_wz_radps = _env_float("ORCH_DOCKING_APPROACH_MAX_WZ", CONFIG.docking.approach_max_wz_radps)
CONFIG.docking.final_max_vx_mps = _env_float("ORCH_DOCKING_FINAL_MAX_VX", CONFIG.docking.final_max_vx_mps)
CONFIG.docking.final_max_vy_mps = _env_float("ORCH_DOCKING_FINAL_MAX_VY", CONFIG.docking.final_max_vy_mps)
CONFIG.docking.final_max_wz_radps = _env_float("ORCH_DOCKING_FINAL_MAX_WZ", CONFIG.docking.final_max_wz_radps)
