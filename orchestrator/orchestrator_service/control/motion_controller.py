...
class MotionController:
    ...
    def _summary(
        ...
        *,
        ...
        semantic_fields = self._semantic_summary_fields(obs) if table_like_obs else {
            ...
            "table_bbox_hold_age_frames": 0,
            "table_bbox_xyxy": None,
            "table_bbox_area_ratio": 0.0,
        ...
        return {
            ...
            **semantic_fields,
            ...
            "table_roi_depth_valid": bool(getattr(obs, "table_roi_depth_valid", False)) if obs is not None else False,
            "table_roi_depth_p10": getattr(obs, "table_roi_depth_p10", None) if obs is not None else None,
            "table_roi_depth_median": getattr(obs, "table_roi_depth_median", None) if obs is not None else None,
            "table_roi_depth_valid_ratio": getattr(obs, "table_roi_depth_valid_ratio", None) if obs is not None else None,
            "table_roi_depth_sample_count": getattr(obs, "table_roi_depth_sample_count", None) if obs is not None else None,
            "table_roi_depth_bbox": getattr(obs, "table_roi_depth_bbox", None) if obs is not None else None,
            "table_roi_depth_bbox_norm": getattr(obs, "table_roi_depth_bbox_norm", None) if obs is not None else None,
            "table_roi_depth_coord_space": getattr(obs, "table_roi_depth_coord_space", None) if obs is not None else None,
    ...
    def _scaled_turn(self, x_abs: float) -> float:
        ...
        lo = float(self.car_cfg.fallback_align_turn_wz_min_radps)
        hi = float(self.car_cfg.fallback_align_turn_wz_max_radps)
        return lo + (hi - lo) * x_abs

    def _scaled_forward(self, distance_ratio: float) -> float:
        ratio = self._clamp(distance_ratio, 0.0, 1.0)
        lo = float(self.car_cfg.fallback_forward_vx_mps_min)
        hi = float(self.car_cfg.fallback_forward_vx_mps_max)
    ...
    def stop_cmd(self, mode: str = "STOP", brake: bool = False) -> MotionDecision:
        ...
        if mode in {"FREEZE_BASE", "GRASP"}:
            cmd.base_freeze = True
    ...
    def _resolve_forward_depth(self, obs: Optional[TableEdgeObs], ctx: Any) -> Dict[str, Any]:
        ...
        current_source = ""
        try:
            table_depth_p10 = getattr(obs, "table_roi_depth_p10", None) if obs is not None else None
            current_depth = float(table_depth_p10) if table_depth_p10 is not None else None
        except (TypeError, ValueError):
            current_depth = None
        if current_depth is not None and current_depth > 0.0:
            current_source = "current_table_roi_depth_p10"
        else:
            current_depth = None

        current_age_s = obs_age_s if obs_age_s is not None else 0.0
        if current_depth is not None and current_age_s <= hold_s:
            sample_mono = now_mono - current_age_s
            ...
            return {
                ...
                "depth_age_ms": current_age_s * 1000.0,
    ...
    def apply_canonical_approach_profile(
        ...
        exit_limit = max(
            hard_limit,
            abs(float(getattr(self.car_cfg, "yolo_forward_center_exit_limit", hard_limit + 0.03) or (hard_limit + 0.03))),
        )
        active_limit = exit_limit if bool(getattr(ctx, "bbox_forward_allowed_latched", False)) else hard_limit
        ...
        if not bbox_valid or abs(float(center_error)) > active_limit or safety_stop:
            return decision
    ...
    def yolo_table_search_cmd(
        ...
        *,
        ...
        control_source: str = "yolo_forward",
    ) -> MotionDecision:
        ...
        summary.update(
            {
                ...
                "fallback_action": "yolo_assist" if source_name in {"yolo_forward", "yolo_track_forward"} else "",
    ...
    def leave_edge_cmd(self) -> MotionDecision:
        self._reset_fallback_memory()
        cmd = self._cmd("LEAVE_EDGE", vx=float(self.car_cfg.leave_edge_vx_mps))
    ...
    def relocate_cmd(self, turn_sign: int = 1) -> MotionDecision:
        self._reset_fallback_memory()
        wz = float(self.car_cfg.relocate_turn_wz_radps) * (1.0 if int(turn_sign) >= 0 else -1.0)
    ...
    def edge_slide_search_cmd(
        ...
        if float(elapsed_s) < float(self.cfg.edge_slide_pause_s):
            ...
            summary.update(
                {
                    "slide_vy_mps": float(self.car_cfg.edge_slide_vy_mps),
                    "weak_slide_vy_mps": float(self.car_cfg.edge_slide_weak_vy_mps),
                    "vx_from_dist": 0.0,
                    "wz_from_yaw": 0.0,
                    "final_vx": 0.0,
                    "final_vy": 0.0,
        ...
        base_vy = float(self.car_cfg.edge_slide_vy_mps if vy_mps is None else vy_mps)
        ...
        if edge_obs is not None:
            ...
            vx_from_dist = self._clamp(
                ...
                -abs(float(self.car_cfg.edge_slide_max_vx_mps)),
                abs(float(self.car_cfg.edge_slide_max_vx_mps)),
            )
            wz_from_yaw = self._clamp(
                yaw_err * float(self.car_cfg.edge_slide_yaw_kp_radps_per_rad),
                -abs(float(self.car_cfg.edge_slide_max_wz_radps)),
                abs(float(self.car_cfg.edge_slide_max_wz_radps)),
            )
            vx = vx_from_dist
            wz = wz_from_yaw
    ...
    def avoid_cmd(self, turn_dir: Optional[str]) -> MotionDecision:
        ...
        if turn_dir in {"left", "l"}:
            wz = abs(float(self.car_cfg.avoid_turn_wz_radps))
        elif turn_dir in {"right", "r"}:
            wz = -abs(float(self.car_cfg.avoid_turn_wz_radps))
        elif turn_dir in {"back", "backward", "reverse"}:
            vx = -abs(float(self.car_cfg.avoid_reverse_vx_mps))
        else:
            wz = abs(float(self.car_cfg.avoid_turn_wz_radps))
    ...
    def _semantic_summary_fields(self, obs: Optional[TableEdgeObs]) -> Dict[str, Any]:
        """Canonical control-facing semantic fields.

        Keep old ambiguous vocabulary out of the main control path.  A few
        compatibility aliases are exported at the end only for older log readers.
        """
        sem = self._table_semantics(obs)
        bbox_area = float(self._yolo_table_bbox_area_ratio(obs))
        return {
            # Table bbox semantics
            "table_bbox_current_found": bool(sem.table_bbox_current_found),
            "table_bbox_control_valid": bool(sem.table_bbox_control_valid),
            "table_bbox_hold_active": bool(sem.table_bbox_hold_active),
            "table_bbox_hold_age_frames": int(sem.table_bbox_hold_age_frames),
            "table_bbox_xyxy": sem.table_bbox_xyxy,
            "table_bbox_area_ratio": bbox_area,
            "table_bbox_conf_raw": sem.table_bbox_conf_raw,
            "table_bbox_conf_used_for_gate": False,

            # Edge perception semantics
            ...
            "edge_stable_count": int(sem.edge_stable_count),
            "edge_conf_raw": sem.edge_conf_raw,
            ...
            # Control eligibility
            "docking_enabled_by_table_bbox": bool(sem.table_bbox_control_valid),
            ...
            # Deprecated compatibility aliases.  Do not use these for new logic.
            "table_bbox_found": bool(sem.table_bbox_current_found),
            "yolo_table_control_valid": bool(sem.table_bbox_control_valid),
            "yolo_table_bbox_area_ratio": bbox_area,
            "yolo_reliable": bool(sem.table_bbox_control_valid),
    ...
    # All generated CmdVel values are now physical speeds (m/s, rad/s).
    ...
    def _limit_table_cmd(self, vx: float, vy: float, wz: float) -> Tuple[float, float, float]:
        ...
        wz_max = max(
            abs(float(getattr(self.car_cfg, "table_wz_view_max_radps", 0.10))),
            abs(float(getattr(self.car_cfg, "table_wz_plane_max_radps", 0.14))),
            abs(float(getattr(self.car_cfg, "table_stage_a_wz_radps", 0.08))),
        )
    ...
    def _compose_coarse_align_cmd(
        ...
        *,
        phase: str = "",
        reason: str = "edge_adjust_yaw_only",
    ) -> MotionDecision:
        sem = self._table_semantics(obs)
        if not sem.table_bbox_control_valid:
            decision = self.search_table_cmd()
            decision.control_summary.update(
                {
                    "control_source": "local_rotate_search",
                    "control_intent": "search",
                    "forward_block_reason": "table_bbox_unavailable",
                    "rotate_block_reason": "",
                }
            )
            return decision
        if not sem.edge_trusted:
        ...
        if stale_level == "fresh" and yaw_abs <= hard_rotate_only_yaw and dist_err > final_lock_tol:
            decision = self._compose_fov_table_cmd(
                obs,
                "EDGE_ADJUST",
                phase=phase or "PLANE_APPROACH",
                reason="edge_guided_forward_from_coarse_align",
            )
            if decision.control_summary is not None:
                decision.control_summary.update(
                    {
                        "control_source": "edge_guided_forward",
                        "approach_source": "edge_guided_forward",
                        "coarse_align_forward_override": True,
                        "hard_rotate_only_yaw_rad": float(hard_rotate_only_yaw),
                        "target_dist_m": float(target_dist),
                        "forward_block_reason": "",
                    }
                )
            return decision
        forward_block_reason = "yaw_too_large_rotate_only" if yaw_abs > hard_rotate_only_yaw else "edge_adjust_yaw_only"
        coarse_align_reason = "edge_trusted_yaw_adjust"
        wz = 0.0
        ...
        if abs(float(wz)) > 1e-9 and not bool(rotate_gate.get("allow_rotate", False)):
            ...
            return self.yolo_table_search_cmd(
                obs,
                mode="CONTROLLED_APPROACH",
                reason=str(rotate_gate.get("rotate_block_reason") or "edge_adjust_rotate_blocked"),
                control_source="yolo_forward",
            )
        ...
        summary.update(
            {
                ...
                "approach_source": "edge_adjust",
                "control_source": "edge_adjust",
                "control_intent": "posture_adjust",
                ...
                "allow_forward": False,
                "allow_rotate": bool(abs(float(wz)) > 1e-9),
    ...
    def _compose_fov_table_cmd(
        ...
        *,
        ...
        if obs is not None and obs.dist_err_m is not None:
            try:
                near_measured = (float(self.cfg.table_target_dist_m) + float(obs.dist_err_m)) <= float(getattr(self.cfg, "yolo_table_near_dist_m", 0.45) or 0.45)
            except Exception:
                near_measured = False
        ...
        if obs is not None and getattr(obs, "view_err_norm", None) is not None:
            try:
                ...
                guard_view_err = view_err
        fov_guard_reason = ""
        if abs(float(guard_view_err)) > hard_th:
        ...
        if base_wz is not None:
            ...
            wz_from_plane = yaw_err * float(getattr(self.car_cfg, "table_plane_yaw_kp_radps_per_rad", 0.60))
        ...
        if view_valid_for_forward and phase_name not in {"PLANE_ACQUIRE", "PLANE_STOP"}:
            ...
            vy_from_view = self._clamp(
                vy_from_view,
                -abs(float(getattr(self.car_cfg, "table_vy_max_mps", 0.02))),
                abs(float(getattr(self.car_cfg, "table_vy_max_mps", 0.02))),
            )

        sem = self._table_semantics(obs, stale_level=stale_level)
        control_source = "edge_guided_forward" if sem.edge_trusted else ("yolo_track_forward" if sem.table_bbox_control_valid else "local_rotate_search")
        ...
        approach_allow_wz = bool(getattr(self.car_cfg, "table_approach_allow_wz", True))
        ...
        if hard_guard:
            ...
            recover_vy = abs(float(getattr(self.car_cfg, "table_view_recover_vy_mps", 0.008) or 0.008))
            recover_wz = abs(float(getattr(self.car_cfg, "table_view_recover_wz_radps", 0.04) or 0.04))
            recover_vy = min(recover_vy, abs(float(getattr(self.car_cfg, "table_vy_max_mps", 0.02) or 0.02)))
            recover_wz = min(recover_wz, abs(float(getattr(self.car_cfg, "table_wz_view_max_radps", 0.08) or 0.08)))
        ...
        pose_missing_safe_vx_mps = abs(float(getattr(self.car_cfg, "table_pose_missing_safe_vx_mps", getattr(self.cfg, "min_forward_vx_mps", 0.04)) or getattr(self.cfg, "min_forward_vx_mps", 0.04)))
        ...
        if valid_for_safe_forward:
            if pose_missing_duration_s <= pose_missing_hold_s:
                vx = max(float(vx), pose_missing_safe_vx_mps)
        ...
        if approach_mode_active:
            if approach_stale:
            ...
            elif yaw_abs > hard_rotate_only_yaw and bool(getattr(obs, "hard_yaw_rotate_only_active", False)):
                vx = 0.0
                vy = 0.0
                forward_block_reason = "yaw_too_large_rotate_only"
                approach_speed_mode = "blocked_yaw_too_large_rotate_only"
                ...
                if yaw_abs <= approach_yaw_deadband:
                    ...
                    approach_speed_mode = "edge_guided_forward_yaw_watch"
                    if not approach_allow_wz:
                        wz = 0.0
                control_source = "edge_guided_forward" if sem.edge_trusted else control_source
                forward_block_reason = ""
        ...
        if (
            self._yolo_reliable(obs)
            and not sem.edge_trusted
            and abs(float(wz)) > 1e-9
            and not bool(rotate_gate.get("allow_rotate", False))
        ):
            wz = 0.0
            speed_summary["wz_radps"] = 0.0
            if control_source == "edge_guided_forward":
        ...
        if approach_mode_active:
            ...
            if not approach_allow_wz:
                wz = 0.0
                speed_summary["wz_radps"] = 0.0
        if abs(vx) < float(abs(float(getattr(self.car_cfg, "table_vx_deadband_mps", 0.004) or 0.004))):
            vx = 0.0
        if abs(vy) < float(abs(float(getattr(self.car_cfg, "table_vy_deadband_mps", 0.003) or 0.003))):
            vy = 0.0
        if abs(wz) < float(abs(float(getattr(self.car_cfg, "table_wz_deadband_radps", 0.006) or 0.006))):
            wz = 0.0
        ...
        summary.update(
            {
                ...
                "vx_mps_min": float(vx_min),
                "vx_mps_max": float(vx_max),
                "vx_kp_mps_per_m": float(vx_kp),
    ...
    def target_track_cmd(self, obs: Optional[TargetObs]) -> MotionDecision:
        ...
        explicit = self._extract_velocity_triplet(obs.vx_mps, obs.vy_mps, obs.wz_radps)
        if explicit is None:
            cmd = self._cmd("EDGE_SLIDE_SEARCH")
    ...
    def return_cmd(self, obs: HomeTagObs) -> MotionDecision:
        ...
        if abs(yaw) > 0.18:
            wz_mag = float(self.car_cfg.return_turn_wz_min_radps) + (
                float(self.car_cfg.return_turn_wz_max_radps) - float(self.car_cfg.return_turn_wz_min_radps)
            ) * min(1.0, abs(yaw))
        ...
        vx = float(self.car_cfg.return_vx_mps_min) + (
            float(self.car_cfg.return_vx_mps_max) - float(self.car_cfg.return_vx_mps_min)
        ) * ratio
